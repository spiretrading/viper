...
download_and_extract() {
  ...
  local build_func="$4"
  local archive="${url##*/}"
  if [[ -f "$folder/.viper_build_complete" ]] &&
      [[ "$(< "$folder/.viper_build_complete")" == "$build_hash" ]]; then
    return 0
  fi
  rm -f "$folder/.viper_build_complete" || return 1
  ...
  echo "$build_hash" > "$folder/.viper_build_complete" || return 1
  rm -f "$archive"
}
